...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
  ...
  <a href="<?= url('/admin/vehicles/create') ?>" class="k-btn k-btn-grad w-full sm:w-auto justify-center"><i data-lucide="plus" class="w-4 h-4"></i> Nuevo vehículo</a>
  <?php endif; ?>
</div>
...
<div x-data="{view: localStorage.getItem('kyros-units-view') || 'grid', set(v){ this.view=v; localStorage.setItem('kyros-units-view',v); }}">
  <!-- Filters + toggle -->
  <form method="GET" class="card p-4 mb-5 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-6 gap-3 items-end">
    ...
    <?php endif; ?>
    <button class="k-btn k-btn-dark !h-10 w-full justify-center">Filtrar</button>
    <a href="<?= url('/admin/vehicles') ?>" class="k-btn k-btn-outline !h-10 w-full justify-center">Limpiar</a>
    <div class="sm:col-span-2 xl:col-span-1 xl:ml-auto flex items-center justify-center gap-1 p-1 rounded-xl bg-paper border hairline">
  ...
  <div x-show="view==='list'" x-cloak class="card overflow-hidden divide-y hairline">
    <?php foreach ($vehicles as $v): ?>
    <div class="flex flex-col sm:flex-row sm:items-center gap-4 p-3.5 hover:bg-paper transition">
      <div class="w-full sm:w-24 h-40 sm:h-16 rounded-xl bg-paper grid place-items-center overflow-hidden shrink-0">
      ...
      <div class="text-left sm:text-right shrink-0 sm:w-24 w-full">
